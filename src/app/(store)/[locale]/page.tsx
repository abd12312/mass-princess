...
export default async function HomePage({ params }: { params: { locale: string } }) {
  ...
  const [{ data: featured }, { data: latest }, { data: cats }, { data: budget }] =
    await Promise.all([
    ...
    <div>
      ...
      <section className="bg-gradient-to-b from-brand-50 to-white">
        <div className="mx-auto flex max-w-6xl flex-col items-center px-4 py-14 text-center">
          ...
          <p className="max-w-md text-2xl font-bold text-gray-700">{t.tagline}</p>
              ...
              >
                <div className="aspect-[4/3] overflow-hidden bg-white">
                  {c.image ? (
                    ...
                    <div className="flex h-full items-center justify-center text-5xl transition group-hover:scale-110">
        ...
        <section className="mx-auto max-w-6xl px-4 py-10">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="text-2xl font-bold">
              {t.under3} <span className="text-gold">✦</span>
            </h2>
            <Link href={`/${locale}/products`} className="text-sm font-semibold text-brand-600">
              {t.viewAll} ←
            </Link>
